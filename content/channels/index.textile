...
Namespaces can be used to apply operations to all channels within the namespace, such as "capabilities":/auth/capabilities, "channel rules":#rules and "integration rules":/general/integrations. Namespaces don't need to be defined for a set of channels to be referred to in a capability. A resource specifier of foo:* (a glob expression) matches channel name foo:bar, even without foo namespace.
